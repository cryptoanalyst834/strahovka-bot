...
const outsideTriggers = [
  ...
  "ДМС","СТРАХОВАНИЕ БИЗНЕСА","ЗАСТРАХОВАТЬ БИЗНЕС","МЕДИЦИНСКАЯ СТРАХОВКА",
  "УЩЕРБ/ТОТАЛ/УГОН","УГОН/ТОТАЛ",
  "подходит для банка",
...
const blockedTopics = [
  "ПОЛИТИКА","ЦЕРКОВЬ","РЕЛИГИЯ","ВОЙНА","ВЫБОРЫ",
...
bot.on('text', async ctx => {
  ...
  if (/КАК ОФОРМИТЬ ОСАГО|ОСАГО ДЛЯ ФИЗИЧЕСКИХ ЛИЦ/.test(up)) {
    return ctx.replyWithHTML(
      `Оформите ОСАГО онлайн — быстро и удобно. Чтобы надежно защитить себя от водителей без ОСАГО или с поддельными полисами, рекомендуем оформить КАСКО от Бесполисных — это быстрый и удобный способ получить расширенную защиту.
...
▶ Оформить`,
      {
        reply_markup: {
          inline_keyboard: [[
            { text:'▶ Оформить', url:services['Ипотека'] }
          ]]
        }
      }
    );
  ...
  // 5) Имущество (личное)
  if (/КАК ОФОРМИТЬ СТРАХОВКУ ИМУЩЕСТВФ|ЗАСТРАХОВАТЬ ИМУЩЕСТВО/.test(up)) {
    return ctx.replyWithHTML(
