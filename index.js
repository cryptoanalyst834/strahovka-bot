...
const outsideTriggers = ["КАСКО ПО РИСКАМ","ТОТАЛ","УГОН","ДМС","СТРАХОВАНИЕ БИЗНЕСА","КАСКО ПО РИСКАМ","УЩЕРБ/ТОТАЛ/УГОН","УГОН/ТОТАЛ","СТРАХОВАНИЕ ЖИЗНИ","МЕДИЦИНСКАЯ СТРАХОВКА","ДОБРОВОЛЬНОЕ МЕДИЦИНСКОЕ СТРАХОВАНИЕ"];
const outsideResp = `
...
`.trim();
const blockedTopics = ["ПОЛИТИКА","ЦЕРКОВЬ","РЕЛИГИЯ","ВОЙНА","ВЫБОРЫ","ЛЕЧЕНИЕ","ЛЕКАРСТВО","ВРАЧ"];
const blockedResp = `
...
bot.on('text', async ctx => {
  ...
  if (services[txt]) {
    return ctx.replyWithHTML(
      ...
      {
        reply_markup:{
          inline_keyboard:[[ { text:'▶ оформить', url:services[txt] } ]]
        }
      }
    );
  }
...
Контакты:
 ...
 Если возникли вопросы или сложности с оформлением через виджет — обращайтесь в поддержку на странице виджета или пишите`,
  ...
  // 2) ОСАГО для юрлиц
  if (/КАК ОФОРМИТЬ ОСАГО ДЛЯ ЮР[ЛИЦ]?|ОСАГО ДЛЯ ЮРИДИЧЕСКИХ ЛИЦ/.test(up)) {
    return ctx.replyWithHTML(
