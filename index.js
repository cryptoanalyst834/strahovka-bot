...
Контакты:
 ...
 📧 info@straxovka-go.ru`,
      {
        reply_markup:{
          inline_keyboard:[[ { text:'▶', url:services['ОСАГО'] } ]]
        }
      }
    );
  }
  ...
  // 2) ОСАГО для юрлиц
  if (/КАК ОФОРМИТЬ ОСАГО ДЛЯ ЮРИДИЧЕСКИХ ЛИЦ|ОСАГО ДЛЯ ЮРИДИЧЕСКИХ ЛИЦ/.test(up)) {
    return ctx.replyWithHTML(
...
Оформление через онлайн-виджет:`,
  ...
  if (/ИПОТЕК|КАК ОФОРМИТЬ ИПОТЕКУ/.test(up)) {
    return ctx.replyWithHTML(
      `Страхование ипотеки — защита рисков имущества, жизни и титула с выгодой 10% для клиентов Сбербанка.
▶ Оформить ипотеки`,
      {
        reply_markup:{
          inline_keyboard:[[ { text:'▶', url:services['Ипотека'] } ]]
        }
      }
    );
  }
  ...
  if (/КАК ЗАСТРАХОВАТЬ ИМУЩЕСТВО|ИМУЩЕСТВО/.test(up)) {
    return ctx.replyWithHTML(
      `Страхование имущества — личное. Подходит для квартиры, дома и т. д.
▶ Оформить`,
      {
        reply_markup:{
          inline_keyboard:[[ { text:'▶', url:services['Страхование имущества'] } ]]
        }
      }
    );
  }
...
▶ Оформить`,
  ...
  if (/ПУТЕШЕСТВИЯ|КАК ОФОРМИТЬ ПОЛИС ДЛЯ ПУТЕШЕСТВИЙ/.test(up)) {
    return ctx.replyWithHTML(
      `Для путешествий за границу оформляйте полис через виджет:
▶ Оформить полис для путешествий`,
      {
        reply_markup:{
          inline_keyboard:[[ { text:'▶', url:services['Путешествия'] } ]]
        }
      }
    );
  }
