...
bot.on('text', ctx => {
  ...
  // 1) ОСАГО для физлиц
  if (/КАК ОФОРМИТЬ ОСАГО|ОФОРМИТЬ ОСАГО|ОСАГО ДЛЯ ФИЗИЧЕСКИХ/.test(up)) {
    return ctx.replyWithHTML(
...
Если возникли вопросы или сложности с оформлением через виджет — обращайтесь в поддержку.`,
  ...
  // 2) ОСАГО для юрлиц
  if (/ОСАГО ДЛЯ ЮРИДИЧЕСКИХ ЛИЦ|Страх ОСАГО ЮР|КАК ОФОРМИТЬ ОСАГО ДЛЯ ЮР/.test(up)) {
    return ctx.replyWithHTML(
...
▶ Оформить ипотеку`,
  ...
  // 5) Личное имущество
  if (/КАК ЗАСТРАХОВАТЬ ИМУЩЕСТВО|СТРАХ ИМУЩ|ИМУЩЕСТВО/.test(up)) {
    return ctx.replyWithHTML(
...
▶ Оформить онлайн`,
  ...
  if (/КОММЕРЧЕСКОЕ ИМУЩЕСТВО|СТРАХОВАНИЕ ИМ|КАК ЗАСТРАХОВАТЬ КОММЕРЧ/.test(up)) {
    return ctx.replyWithHTML(
...
Контакты:
 ...
 📧 info@straxovka-go.ru`,
  ...
  // 7) Путешествия
  if (/ПУТЕШЕСТВИЯ|ЗАСТРАХОВАТЬ ПУТЕШЕСТВ|КАК ОФОРМИТЬ ПОЛИС ДЛЯ ПУТЕШЕСТВИЙ/.test(up)) {
    return ctx.replyWithHTML(
